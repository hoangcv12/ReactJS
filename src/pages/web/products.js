...
import {  pageProducts } from 'redux/actions/product';
...
export default function ProductsWeb() {
  ...
  const img = (data,idcate) => {
    if (idcate === "1") {
      ...
      return <a href onClick={() => history.push(`/products/${data}/detail`)} ><img className="card-img-top" src="https://webonline.macstore.mx/img/sku/WATCH277_FZ.jpg" alt /></a>
    } else {
...
// var list = [{name:"hoàng",age:"10"},{name:"hân",age:"10"}]
      ...
      <div className="row ">
        {products.map((product, index) => (
          <div className="col-3 p-2 px-md-5">
            ...
            {img(`${product.id}`,`${product.idcate}`)}
              <div className="card-body">
                <h5 className="card-title">{product.name}</h5>
                ${product.price}
                <i className="card-text ml-5 col "  >{product.status === "true" ? (
